---
title: "Problemas de configuração do GraphQl do AEM"
...
# Problemas de configuração do GraphQl no AEM
...
O cliente está desenvolvendo um site que deve recuperar dados de suas instâncias do AEM usando a solução AEM headless.

Para configurar o endpoint, consulte o tutorial em `[` 1`]`  foi seguido.

Após a publicação de todos os fragmentos de conteúdo, todos os modelos, conteúdo, referências, ativos e consultas conforme descrito em `[` 2`]` , ao tentar recuperar qualquer fragmento de conteúdo da instância de publicação, um erro 404 é recebido, impedindo que o conteúdo seja lido do editor.
...
`[` 1`]`  aem_base_version: 6.5 aem_envId: xxxxxxx aem_envType: dev aem_program_id: xxxxxx aem_release_id: xxxxx aem_service: cm-pxxxxxx-exxxxxx aem_tenant: ns-team-aem-cm-prd-nxxxxxx aem_tier: publicar back-end: publishfarm/- cache: <b>bloqueado</b>
cluster: `<` cluster`>`
código: - caminho_do_arquivo: /var/log/aem/dispatcher.log host: publish-pxxxxxx-exxxxxxx.adobeaemcloud.com método: POST namespace: ns-team-aem-cm-prd-nxxxxxx orig_time: 25/Jul/2023:15:39:11 +0000 caminho: /content/cq:graphql/graphql/endpoint.json nome_do_pod: `<` pod_name`>`
pod_uid: `<` pod_uid`>`
