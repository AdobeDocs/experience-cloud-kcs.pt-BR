...
O objetivo deste artigo é explicar detalhadamente como solucionar todos os problemas comuns de indexação.
...
- Enquanto a reindexação estiver em andamento, o atributo de “`status`” `IndexStatsMBean` teria o valor “`running`”.
...
A atividade de índice assíncrona pode ser identificada no nível INFO ao continuar com `org.apache.jackrabbit.plugins.index.IndexUpdate` e `org.apache.jackrabbit.plugins.index.AsyncIndexUpdate`.
...
- Definir a propriedade booleana `saveDirectoryListing=true` (somente se estiver usando o Oak 1.0.16, se a versão posterior não for necessária) e defina a propriedade `excludedPaths=/var`, `/etc/workflow/instances`, `/jcr:system` no Índice OOTB do `/oak:index/lucene` para otimizá-lo.
- Clique em “Salvar tudo”
- Ao criar índices de propriedade do lucene personalizados, defina `saveDirectoryListing=true` (Booleano) se estiver usando Oak 1.0.16 e defina `includedPaths` (String) para restringir seu índice para indexar apenas determinados caminhos.
