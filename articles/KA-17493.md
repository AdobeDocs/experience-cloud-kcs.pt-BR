...
[&lt;- Anterior: Uso e noções básicas sobre variáveis](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-17487.html)
...
![O autor ativa o conteúdo, o que aciona o editor para enviar uma solicitação de liberação para o dispatcher](assets/dispatcher-flushing-order-of-events.png "dispatcher-flush-order-of-events")
...
![captura de tela do agente de replicação padrão da página da Web AEM /etc/replication.html](assets/author-rep-agent-example.png "author-rep-agent-example")
...
![captura de tela do agente de replicação de liberação padrão da página da Web AEM /etc/replication.html](assets/publish-flush-rep-agent-example.png "publish-flush-rep-agent-example")

### REPLICAÇÃO DE LIBERAÇÃO DO DISPATCHER RECEBENDO HOST VIRTUAL

O módulo Dispatcher procura cabeçalhos específicos para saber quando uma solicitação POST é algo a ser transmitido para renderizações de AEM ou se é serializado como uma solicitação de liberação e precisa ser manipulado pelo próprio manipulador do Dispatcher.

Esta é uma captura de tela da página de configuração que mostra estes valores:
![imagem da guia de configurações da tela de configuração principal com o Tipo de serialização mostrado como Liberação do Dispatcher](assets/disp-flush-agent1.png "disp-flush-agent1")

A página de configuração padrão mostra o `Serialization Type` as `Dispatcher Flush` e define o nível de erro

![Captura de tela da guia de transporte do agente de replicação.  Isso mostra o URI para publicar a solicitação de liberação.  /dispatcher/invalidate.cache](assets/disp-flush-agent2.png "disp-flush-agent2")

No `Transport` guia você pode ver a variável `URI` sendo definido para apontar o endereço IP do dispatcher que receberá as solicitações de liberação.  O caminho `/dispatcher/invalidate.cache` O não é como o módulo determina se é uma liberação, é apenas um terminal óbvio que você pode ver no log de acesso para saber se foi uma solicitação de liberação.  No `Extended` guia vamos analisar as coisas que estão lá para qualificar que esta é uma solicitação de liberação para o módulo do dispatcher.

![Captura de tela da guia Extended do agente de replicação.  Observe os cabeçalhos enviados com a solicitação POST para avisar o dispatcher sobre a liberação](assets/disp-flush-agent3.png "disp-flush-agent3")
...
![Tela de um agente de replicação padrão mostrando que o agente de replicação reage e aciona quando novos itens foram recebidos de um evento de replicação do conteúdo de publicação do autor](assets/disp-flush-agent4.png "disp-flush-agent4")
...
![arquivos de status misturados ao conteúdo e datas mostradas com os níveis de stat mostrados](assets/dispatcher-statfiles.png "dispatcher-statfiles")
...
[Próximo -> URLs personalizados](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-17463.html)