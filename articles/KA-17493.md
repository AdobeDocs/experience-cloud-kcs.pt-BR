...
A página de configuração padrão mostra o <b>[!UICONTROL Tipo de serialização]</b> as *[!UICONTROL Liberação do Dispatcher]* e define o nível de erro
...
O [!UICONTROL Método HTTP] para solicitações de liberação é apenas uma solicitação GET com alguns cabeçalhos de solicitação especiais:
...
- Host
   - É aqui que a variável `Host` O cabeçalho é falsificado para direcionar um `VirtualHost` que está configurado no dispatcher [!DNL Apache] servidor da Web (`/etc/httpd/conf.d/enabled_vhosts/aem_flush.vhost`).  É um valor codificado que corresponde a uma entrada no `aem_flush.vhost` do arquivo *`ServerName`* ou *`ServerAlias`*
...
No [!UICONTROL Triggers] guia observaremos os acionadores alternados que usamos e o que eles são

- [!UICONTROL Ignorar padrão]
...
- [!UICONTROL Sem controle de versão]
...
<br><br><br><br><br><br>O `.stat` arquivo<br><br><br><br><br><br>
...
Observe que em cada diretório havia um `.stat` arquivo presente.  Este é um indicador de que ocorreu uma liberação.  No exemplo acima de <b>`statfilelevel`</b> foi definida como <b>`3`</b> dentro do arquivo de configuração farm correspondente.
...
Esse diretório deve corresponder ao [!DNL Apache] configuração raiz do documento para o domínio que seu servidor da Web está configurado para usar.

Ter pastas docroot aninhadas para cada farm que vivem subpastas do [!DNL Apache] a raiz do documento é uma péssima ideia por muitas razões.
...
Depois de disparar o comando de solicitação para o dispatcher, você desejará ver o que é feito nos logs e o que é feito com o `.stat files`.  Siga o arquivo de log e você deve ver as seguintes entradas para confirmar a ocorrência da solicitação de liberação do módulo do dispatcher
<br><br><br><br><br> <br><br><br><br>
