...
<br><br><br>Como posso obter despejos de threads de uma JVM no Linux, UNIX ou Windows?<br><br><br><br> <br><br>
...
<br><br>Etapa 1: Obter o PID do seu processo Java<br><br><br><br> <br><br>
...
<b>Observação:</b> No Linux e UNIX, talvez seja necessário executar esse comando como sudo -u user jps -l, onde &quot;user&quot; é o nome de usuário do usuário no qual o processo Java está sendo executado.
...
Para obter um despejo de thread usando jstack, execute o seguinte comando: jstack -l pid

Você pode enviar dumps de encadeamento consecutivos para um arquivo usando a diretiva de redirecionamento/anexação de saída do console: jstack -l pid threaddumps.log
...
- Se o comando jstack (jstack -l pid threaddumps.log) gerar o erro 1 abaixo, execute o comando como o usuário proprietário do processo java.  Por exemplo:
...
Faça assim: sudo - u *usuário* jstackSeries.sh *pid* *aemserveruser* *count* *atraso*
...
1. Execute este comando para fazer isso:
   ...
   Pode ser necessário executar este comando como sudo -u user kill -QUIT *pid* onde &quot;usuário&quot; é o usuário no qual o processo Java está sendo executado.
2. Se você estiver iniciando o CQSE usando o script crx-quickstart/server/start , seus dumps de encadeamento serão enviados para crx-quickstart/server/logs/startup.log. Se você estiver usando um servidor de aplicativos de terceiros, como JBoss, WebSphere, Tomcat ou outro, consulte a documentação do servidor para descobrir a qual arquivo o resultado padrão é direcionado.
...
1. Adicione o seguinte parâmetro ao jvm executando Communique : -Dcom.sun.management.jmxremote
