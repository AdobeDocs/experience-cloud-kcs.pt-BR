...
O documento descreverá as entradas de log comuns que você verá e o que elas significam e como lidar com elas.<br><br>[Índice](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-17490.html)
...
Since [!DNL dispatcher] o módulo 4.2.x começou a desencorajar as pessoas a usar o seguinte tipo de correspondência em seus arquivos de filtros:
...
Entenda que a variável [!DNL dispatcher]As regras do foram definidas para filtrar essa solicitação. Nesse caso, a página que tentou ser visitada foi rejeitada de propósito e não gostaríamos de fazer nada com isso.
...
Isso ocorre quando você tem o endereço IP incorreto configurado na seção renders do farm. Ou a instância de AEM parou de responder ou escutar e o [!DNL dispatcher] Não consigo alcançá-lo.
...
Isso significa que a instância do AEM tinha um soquete aberto que podia ser acessado e o tempo limite foi atingido com a resposta. Isso significa que a instância do AEM era muito lenta ou não estava funcionando e [!DNL dispatcher] atingiu as configurações de tempo limite definidas na seção de renderização do farm. Aumente a configuração de tempo limite ou deixe a instância de AEM em funcionamento.
...
Esse erro ocorre quando você configurou o [!DNL dispatcher] para usar o filtro automático dinâmico, permita URLs personalizadas, mas não concluiu a configuração instalando o pacote no renderizador de AEM.

Para corrigir isso, instale o pacote de recursos vanityurl na instância do AEM e permita que ele esteja pronto pelo usuário anônimo. Detalhes [here](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-17463.html%3Flang%3Den)
...
A página foi buscada por meio do método http do GET para o conteúdo `/content/we-retail/us/en.html` E levou 24034 milissegundos. A parte a que queríamos prestar atenção está no fim <b>`publishfarm/0`</b>. Você verá que ele direcionou e correspondeu à função `publishfarm`. A solicitação foi obtida do renderizador 0. Isso significa que essa página tinha que ser solicitada AEM e armazenada em cache. Agora vamos solicitar essa página novamente e ver o que acontece com o log.
