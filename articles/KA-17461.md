...
1. &#x200B;<b>private </b>- o <b>private</b> na diretiva <b>Controle de cache</b> faz com que o arquivo seja armazenado em cache somente no navegador, não em caches intermediários, como CDNs.  Um uso prático para esta diretiva seria se sua página incluísse conteúdo personalizado/específico do usuário. 
   ...
   <b>  </b>Controle de cache: max-age=300, private
2. <b>&#x200B; s-maxage </b>- o <b>s-maxage </b>na diretiva <b>Controle de cache</b> permite definir um TTL diferente para caches compartilhados, como CDNs.  Quando esse valor é definido, o navegador usaria o que é definido em <b>max-age</b> e outros caches respeitariam <b>s-maxage</b> em vez disso.
   ...
   <b>  </b>Controle de cache: max-age=600, s-maxage=300
...
2. Respostas de compactação GZip para todos os tipos de arquivo que não são pré-compactados.
   ...
   - 
      ...
      Ative a compactação GZip no nível do Dispatcher:

      - Apache - use [mod_deflate](https://httpd.apache.org/docs/2.4/mod/mod_deflate.html).  Tenha cuidado com o uso do Vary pelo mod_deflate.  Em certos casos, o cabeçalho Vary pode fazer com que a CDN e o navegador ignorem totalmente o armazenamento em cache.
      - Microsoft IIS - use [Compactação dinâmica](https://docs.microsoft.com/en-us/iis/configuration/system.webserver/httpcompression/).
      - 
...
3. Se o seu provedor CDN suportar [Inclusões do lado da borda](https://en.wikipedia.org/wiki/Edge_Side_Includes) (ESI) em seguida, utilize esse recurso.

   - 


      AEM componentes podem ser desmembrados usando o ESI.  Para fazer isso, use o Apache [Inclusões dinâmicas do Sling](https://github.com/Cognifide/Sling-Dynamic-Include) ou implementar uma solução personalizada.
   - 
...
<br><br><br><br> <br><br>AEM Armazenamento em Cache do Dispatcher<br><br><br><br> <br><br>
...
1. Defina os seguintes cabeçalhos de resposta HTTP que não são definidos por AEM por padrão.
   1. <b>[Controle de cache: max-age=...](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Cache-Control) </b>- Para definir esse cabeçalho, [ACS Commons - Dispatcher TTL](https://adobe-consulting-services.github.io/acs-aem-commons/features/dispatcher-ttl/index.html) poderia ser usado ou você poderia implementar um código personalizado para defini-lo.
...
2. Se o site tiver conteúdo personalizado/dinâmico:
   ...
   1. Use [Apache Sling Dynamic Inclues](https://helpx.adobe.com/experience-manager/kt/platform-repository/using/sling-dynamic-include-technical-video-setup.html) para dividir o conteúdo para que diferentes partes da página possam ser armazenadas em cache por diferentes períodos de tempo.
...
3. Otimizar bibliotecas de clientes.
   ...
   3. [Incorporar cliente](https://helpx.adobe.com/experience-manager/6-3/sites/developing/using/clientlibs.html)[bibliotecas](https://helpx.adobe.com/experience-manager/6-3/sites/developing/using/clientlibs.html) para minimizar e reduzir arquivos js e css.
