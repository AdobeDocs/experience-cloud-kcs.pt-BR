...
A fila de replicação é bloqueada periodicamente para um de nossos clientes AEMaaCS. Este artigo descreve a solução alternativa sugerida pela Equipe de engenharia que pode ser executada na interface do usuário usando permissões de administrador.
...
Um de nossos Clientes do AEMaaCS fila de replicação bloqueada periodicamente devido à javax.jcr.NamespaceException: prefixo de namespace desconhecido: exceção ns22_.
...
Para desbloquear a fila, é necessário simplesmente excluir a solicitação da interface do usuário da fila de distribuição golden-publish. Sugira ao cliente que execute as etapas abaixo.
