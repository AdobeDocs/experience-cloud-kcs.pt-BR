...
<br><br>Visão geral<br><br><br>CUIDADO<br><br><br>
Este artigo é fornecido apenas como um guia de exemplo geral. Você deve se envolver com o Gerente de sucesso do cliente da Adobe Campaign para medir o dimensionamento exato da implantação antes de iniciar o [!DNL Campaign] projeto. <b>Não</b> adquirir ou implantar qualquer infraestrutura ou hardware até que isso seja feito.

Este documento fornece recomendações gerais para a implantação do Adobe Campaign Classic v7 no seu data center local ou no ambiente de nuvem virtualizada. Esse tipo de implantação, chamada de <b>híbrido</b> ou <b>mid-sourcing</b>, coloca o [!DNL Campaign] instância de marketing e banco de dados de marketing sob seu controle operacional, ao usar os serviços da Adobe Cloud Messaging para enviar emails, mensagens SMS ou SMPP e coletar dados de rastreamento de cliques, devoluções e aberturas de email.
...
- Os dados de rastreamento de email são retidos na variável [!DNL Campaign] sistema por 90 dias
...
[!DNL Campaign] O é um aplicativo centrado em banco de dados e o desempenho do servidor de banco de dados é essencial. A execução de workflows, segmentação, rastreamento de uploads de dados, interações de entrada, análises e outras atividades geram atividade do banco de dados. Em geral, o tamanho e a frequência dessas operações determinam o tamanho dos servidores de banco de dados.

Os servidores de aplicativos em sua instância de marketing exigem CPU e memória suficientes para executar workflows e responder a chamadas de API SOAP, incluindo solicitações de [!DNL Campaign] Usuários do console. Os requisitos da CPU podem ser significativos para workflows que usam interações de saída com regras de oferta complexas, workflows que executam JavaScript personalizado e aplicativos da Web com altos níveis de tráfego.

[!DNL Campaign] aplicativos web também podem ser implantados nos servidores de aplicativos da instância de marketing ou em sistemas de servidores da Web separados. Como as cargas de trabalho de aplicativos Web estão em conflito com fluxos de trabalho críticos e [!DNL Campaign] Usuários do console, aplicativos da Web e interações de entrada podem ser implantados em servidores separados, para garantir que o [!DNL Campaign] A funcionalidade é executada de forma confiável com bom desempenho.
...
O [!DNL Campaign] A arquitetura impõe alta segurança usando SSL sobre HTTP (HTTPS) para se comunicar entre sua instância de marketing e o Adobe Cloud Messaging. A segurança, confiabilidade e disponibilidade são aplicadas usando proxies reversos em uma sub-rede &quot;zona desmilitarizada&quot; (DMZ) para isolar e proteger os servidores e o banco de dados das instâncias de marketing.
...
Esses sistemas criam a instância de marketing do Servidor de aplicativos, que é compatível diretamente com seu [!DNL Campaign] Usuários do console e executa os workflows da campanha.
...
Os recursos de participação/não participação e centro de preferências de assinatura podem ser fornecidos por [!DNL Campaign] ou pelo seu próprio site. Se optar por implementar essa funcionalidade em seu site, você deve garantir que as informações de preferência e assinatura sejam propagadas para o [!DNL Campaign] banco de dados de marketing. Normalmente, é feito criando arquivos de extração que são carregados por [!DNL Campaign] fluxos de trabalho automaticamente.
...
A implantação que oferece suporte a 50 milhões de recipients é essencialmente a mesma mostrada em [Cenário 2](https://experienceleague.adobe.com/docs/campaign-classic/using/technotes/hardware-sizing.html?lang=en#scenario-2): [!DNL Campaign] o tráfego da aplicação web é roteado para [!DNL Campaign] servidores da Web, de modo que as rastras de tráfego da Web após grandes inicializações de campanha não afetam [!DNL Campaign] fluxos de trabalho e usuários do Console do cliente.
...
Todos [!DNL Campaign] servidores são bons candidatos para virtualização. Vários problemas devem ser abordados para garantir disponibilidade e desempenho adequados.
