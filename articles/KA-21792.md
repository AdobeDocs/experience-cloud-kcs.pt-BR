...
<b>Ambiente</b><br><br>Experience Manager as a Cloud Service<br><br> <br><br><b>Problema/Sintomas</b><br><br>Na versão 2023.02.11382, alguns aprimoramentos foram feitos no <b>GraphQL</b> Implementação da (Open-Source Query Language) que pode causar comportamento inesperado no código do aplicativo, caso tenha dependido do comportamento incorreto das versões mais antigas. 
<br>A resolução mencionada abaixo ajuda a alcançar que o Adobe Experience Manager (AEM) GraphQL agora retorne conteúdo com *null* valores de maneira consistente em todas as operações de filtro. <br>

## Resolução {#resolution}

`CONTAINS_NOT` e *null* values<br> <br>A primeira alteração afeta as condições do filtro aproveitando a `CONTAINS_NOT` operador. Em versões anteriores, essa `CONTAINS_NOT` não retornou fragmentos de conteúdo contendo *null* no campo ao qual a condição de filtro foi aplicada. Isso é inconsistente com operadores similares como `EQUALS_NOT` ou `NOT_AT`.<br>Certifique-se de que o aplicativo cliente é capaz de lidar com *null* valores retornados pelo AEM ao usar `CONTAINS_NOT`.<br> <br><b>Exemplo:</b>
...
<br>Aplicar modos `ALL_OR_EMPTY/ALL` e *null* values<br>
...
<br>Modos de aplicação: `INSTANCES`, `_instances: 0` e *null* values<br>
...
<br>Melhorias que podem afetar os clientes com `ft-sites-97` ativado<br>
...
<br>tratamento nulo com condições que verificam não-igualdade<br>
...
<br>Operador de filtro `NOT_AT` com valores de data/hora<br>
...
<br>Matrizes: `INSTANCES` com `_instances: 0` não funciona corretamente<br>
