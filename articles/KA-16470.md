---
title: "Audience Manager to Campaign"
...
# Audience Manager para Campanha
...
10. Os usuários devem conseguir fazer logon no Campaign V6 via Adobe ID
...
A. Configuração/provisionamento de &#x200B;
...
2. Sincronizar o banco de dados do CRM com o Cliente do Campaign precisa configurar um processo no qual ele envie seus dados do CRM para o Campaign. O email, CRM_ID, são os campos mínimos a serem sincronizados.
...
4. O DTM lê a Configuração do CRM_ID na ferramenta &quot;serviço da Marketing Cloud ID&quot;. O elemento de dados hash_email é configurado no DTM lendo o CRM_ID disponível no dataLayer.
...
O sFTP criado para o AAM precisa ser configurado com o Adobe Campaign para que o Campaign possa enviar e receber segmentos do AAM. As credenciais sFTP serão fornecidas a você pelo Atendimento ao cliente do Adobe ou pelo representante do Adobe. Durante o provisionamento, os locais de entrada e saída do ftp são provisionados e são usados pela campanha para enviar públicos para AAM e segmentos integrados do AAM para a campanha, respectivamente.
...
6. &#x200B; &#x200B; &#x200B; &#x200B; &#x200B; criar as fontes de dados e os destinos com o &#x200B;
...
7. &#x200B; &#x200B; &#x200B; &#x200B; &#x200B; configurar as fontes de dados no Adobe Campaign As fontes de dados são configuradas no Adobe Campaign. Duas fontes de dados serão criadas na interface do usuário do Adobe Campaign; um com base na ID declarada ou na ID do cliente com hash e o outro com base na Marketing Cloud id ou na UUID do solicitante. A ID de destino correspondente a cada ID é adicionada nos respectivos campos. Por exemplo, a ID de destino AAM Adobe Campaign de destino - ID declarada é adicionada na configuração da fonte de dados com a chave de reconciliação como ID do cliente com hash.
...
8. Rastrear o comportamento do usuário no site No caso de uso abaixo, a palavra-chave de pesquisa usada por um cliente é capturada na evar2 e uma compra de produto é um evento de compra no Analytics. O fluxo abaixo é baseado no encaminhamento pelo lado do servidor de chamadas AA para o AAM. Na página de pesquisa, capturamos o parâmetro &quot;q&quot; e o colocamos em um elemento de dados:
...
Em seguida, também capturamos o evento de &quot;compra&quot; no analytics na página de agradecimento:
...
Neste ponto, podemos rastrear nossos clientes e criar segmentos no Audience Manager. Além disso, garantimos que o Audience Manager e o Campaign estão coletando os usuários com a mesma CRM_ID.

9. &#x200B; &#x200B; &#x200B; &#x200B; &#x200B; criar segmentos em &#x200B; com base no comportamento do usuário. Em seguida, crie as seguintes características: &quot;Buscou o inverno&quot;:
...
10. &#x200B; &#x200B; &#x200B; &#x200B; &#x200B; encaminhar o segmento para o Campaign Para compartilhar um público da AMC com a campanha, a configuração deve ser feita no Adobe Campaign assim que o segmento for criado em &#x200B;. Comece criando um público-alvo compartilhado no Adobe Campaign e selecione o segmento AMC na lista de segmentos. Dois públicos-alvo compartilhados devem ser criados com base na MID e na ID declarada na campanha. Quando a configuração for concluída, os segmentos poderão ser vistos como mapeados automaticamente para os respectivos destinos no AAM.
...
11. Compartilhe um público-alvo de campanha com o AAM por meio da AMC Para compartilhar um público-alvo de campanha com o AAM por meio da AMC, a seguinte configuração é necessária no Adobe Campaign. Na opção de menu Administração na interface do usuário da campanha, selecione workflows técnicos de produção. Clique em novos workflows técnicos, arraste a query e, em seguida, atualize o público compartilhado e, em seguida, termine.
...
No campaign, primeiro crie uma lista com base em um público-alvo. Isso está em &quot;listas&quot;:
...
O pop-up agora permite selecionar um público-alvo da Marketing Cloud. Escolha &quot;search_for_inverno_did_not-buy&quot;:
...
Você pode não ter feito logon usando sua Adobe ID para campanha. Se você fez logon usando sua Adobe ID, certifique-se de ter sido adicionado ao produto da Biblioteca de público-alvo na interface do usuário do Marketing Cloud Enterprise Console.
...
3. Nunca mapeie os segmentos para o destino da campanha a partir da interface do usuário do AAM &#x200B;
