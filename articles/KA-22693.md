---
title: "Não é possível selecionar o Adobe Target como a solução em nuvem para uma configuração IMS"
...
# Não é possível selecionar o Adobe Target como a solução em nuvem para uma configuração IMS
...
Ambiente: AEM no local



O cliente está tentando criar uma configuração IMS do Adobe Target para integrar o AEM ao Adobe Target.

No entanto, sempre que eles selecionam &quot;Adobe Target&quot; como a solução em nuvem para a configuração do IMS, o erro &quot;Não é possível recuperar certificados: a userId fornecida não identifica um usuário existente&quot; é exibido e a configuração não pode ser criada.

Esse problema ocorre apenas para a solução da Adobe Target.



Etapas a serem reproduzidas:

1. Ir para Ferramentas `>`  Segurança `>`  Configurações do IMS `>`  Criar
2. Selecionar o Target como solução

Você recebe um erro na interface do usuário &quot;Não é possível recuperar o certificado. A ID de usuário fornecida não identifica um usuário existente.&quot;

Nos logs, o único erro relacionado a esse problema é `[` 1`]` .
...
Ao verificar as permissões para os usuários &quot;target-imsconfig-service&quot; e &quot;targetservice&quot;, foi notado que target-imsconfig-service não tinha ACEs atribuídas.
...
Privilégios: jcr:read; rep:write
