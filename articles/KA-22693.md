...
<br>Causa<br>
O erro é causado pela falta de permissões do `target-imsconfig-service` usuário. Ao verificar as permissões para os usuários do `target-imsconfig-service` e `targetservice,` foi observado que `target-imsconfig-service` não tinha ACEs atribuídos. Depois de adicionar as ACEs `[` 1`]`  Para o usuário, a configuração do IMS para o Adobe Target pode ser criada com sucesso.
