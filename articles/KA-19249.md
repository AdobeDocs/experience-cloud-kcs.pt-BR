...
É possível ver as Referências locais de um ativo na guia [!UICONTROL Referências] das [!UICONTROL Propriedades]. (No Console do [!DNL Assets], selecione um ativo, selecione [!UICONTROL Propriedades] na barra de ferramentas e clique na guia [!UICONTROL Referências].) Ao usar as informações fora do AEM, você pode exportá-las no formato JSON.
...
Acessar o seguinte URL “1” retorna Referências locais no formato JSON. Os valores do `path` na resposta são os caminhos de conteúdo que fazem referência a um ativo de destino.
